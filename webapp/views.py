...
#Getting rid of articles without images
		...
		for x in description_keywords:
			if {'image':x['default_url'],'id':x['id'], 'article_id':x['default_id']} not in images:
				images.append({'image':x['default_url'],'id':x['id'], 'article_id':x['default_id']})
		...
		for x in images:
			title = json.loads(ArticleSearchByID(x['article_id'], hearst_api_key))['items']
			if title != []:
				x['article_title'] = title[0]['title']
				x['publication'] = title[0]['origin_site_name']
				x['paragraph'] = remove_tags(str(title[0]['body'][0]['blog_element'][0]['paragraph']))
				print str(x['article_title']) + '\n\n\n\n\n'
				print str(x['publication']) + '\n\n\n\n\n'
				print str(x['paragraph']) + '\n\n\n\n\n'
			else:
				x['article_title'] = ''
				x['publication'] = ''
